...
def generate_loan():
    ...
    # new way ... traces back to creation time
    linux_ts = time.time()
    loan_id = f"L{linux_ts}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--output_dir", default='/tmp/', type=str,
                        help=f"debug level Debug:{logging.DEBUG} to Info:{logging.INFO}")
    ...
    log.info(f'{source_file} startup')
    ...
    # Generate {number_loan_applications} loan JSON objects
    ...
    for index, loan_json_object in enumerate(loan_objects, start=1):
        ...
        json_fname =f"{output_dir}/{fname_loanId}.json"
        with open(f'{json_fname}', "w") as output_file:
            output_file.write(json.dumps(loan_json_object, indent=2))

        log.debug(f"Loan {index}:\n{json.dumps(loan_json_object, indent=2)}\n")

    log.info(f'Generated {number_loan_applications} Loan Origination JSON files in [{output_dir}]')
